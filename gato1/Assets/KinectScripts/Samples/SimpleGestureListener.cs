...
//		manager.DetectGesture(userId, KinectWrapper.Gestures.SwipeDown);
	                              ...
	                              KinectWrapper.NuiSkeletonPositionIndex joint, Vector3 screenPos)
	{
        ...
        {
            int indexActive = modelos.IndexOf(modelos.Find(item => item.activeInHierarchy));
            int nextToActive = (indexActive + 1) % modelos.Count;
            ...
            menu.ForEach(delegate (GameObject item)
            {
                item.SetActive(false);
            });

            modelos[nextToActive].SetActive(true);
            menu[nextToActive].SetActive(true);

        }
        if (gesture == KinectGestures.Gestures.SwipeRight)
        {
            int indexActive = modelos.IndexOf(modelos.Find(item => item.activeInHierarchy));
            int nextToActive = indexActive == 0 ? modelos.Count -1 : (indexActive - 1) % modelos.Count;
