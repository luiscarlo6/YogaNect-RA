...
public class AvatarCompare : MonoBehaviour
{
    ...
    public UnityEngine.UI.Image felicitacionesImage;
    ...
    public GUIText GesturesText;
    public GUIText felicitacionText;
    ...
    public AvatarController avatarKinect;
    ...
    public void Update()
    {
        ...
        //MoveAvatar(UserID);
        Transform[] ArrayAvatarInMove = avatarKinect.getBones();// solo lo puse para que quitara el error -- implementar ObtainArrayBoneMove();
        int[] parteInferior = {  0,1,2,3,//tronco
                                5, 6, 7, 8, //BrazoIzq
                                10, 11, 12, 13, // BrazoDer 
                                14, 18,//Cadera
                                15, 19//Rodillas
                              };//Tobillo
        bool iguales = true;
        for (var i = 0; i < parteInferior.Length; i++)
        {
            int boneIndex = parteInferior[i];

            if (boneIndex2JointMap.ContainsKey(boneIndex))
            {

                if (!CompareTransform(ArrayAvatarInMove[boneIndex], bones[boneIndex], errorx, errory, errorz))
                {

                    //Debug.ClearDeveloperConsole();
                    iguales = false;
                    GesturesText.text = "Acomoda el " + bones[boneIndex].transform.name;
                    startTime = Time.time;
                    felicitacionText.gameObject.SetActive(false);
                    break;
                }
            }
        ...
        if (iguales)
        {
            ...
            if (t <= 3.1f)
            {
                ...
                if (t < 1.0f)
                {
                    timerImage.sprite = seg0;
                }
                if (t >= 1.0f && t < 2.0f)
                {

                    audio.Play();
                    timerImage.sprite = seg1;
                }
                if (t >= 2.0f && t < 3.0f)
                {
                    audio.Play();
                    timerImage.sprite = seg2;
                }
                if (t >= 3.0f)
                {
                    audio.Play();
                    timerImage.sprite = seg3;
    ...
    protected bool CompareTransform(Transform a, Transform b, float errorx, float errory, float errorz)
    {
        bool bienx = (Math.Abs(a.transform.position.x - b.transform.position.x - 4.0f)) <= errorx;
        bool bieny = Math.Abs(a.transform.position.y - b.transform.position.y) <= errory;
        bool bienz = Math.Abs(a.transform.position.z - b.transform.position.z) <= errorz;
        bool result = bienx && bieny && bienz;
        if (!result)
        {
            Debug.Log("error" + a.transform.name + " " + (a.transform.position - b.transform.position - new Vector3(4, 0, 0)));
        }
    ...
    */
    ...
    protected float normalise(float value)
    {
        float start = -180;
        float end = 180;
